	...
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
				...
				<el-form-item label="标题" prop="title">
							  ...
							  v-if="user_group === '管理员' || (form['message_board_id'] && $check_field('set','title')) || (!form['message_board_id'] && $check_field('add','title'))" :disabled="disabledObj['title_isDisabled']"></el-input>
				...
				<el-form-item label="用户编号" prop="user_no">
							...
							<!--v-if="user_group === '管理员' || (form['message_board_id'] && $check_field('set','user_no')) || (!form['message_board_id'] && $check_field('add','user_no'))" :disabled="disabledObj['user_no_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','user_no')">{{form['user_no']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['message_board_id'] && $check_field('set','user_no')) || (!form['message_board_id'] && $check_field('add','user_no'))" id="user_no" v-model="form['user_no']" :disabled="disabledObj['user_no_isDisabled']">
				...
				<el-form-item label="姓名" prop="full_name">
							  ...
							  v-if="user_group === '管理员' || (form['message_board_id'] && $check_field('set','full_name')) || (!form['message_board_id'] && $check_field('add','full_name'))" :disabled="disabledObj['full_name_isDisabled']"></el-input>
				...
				<el-form-item label="发布时间" prop="release_time">
								<el-date-picker :disabled="disabledObj['release_time_isDisabled']" v-if="user_group === '管理员' || (form['message_board_id'] && $check_field('set','release_time')) || (!form['message_board_id'] && $check_field('add','release_time'))" id="release_time"
						v-model="form['release_time']" type="date" placeholder="选择日期">
					</el-date-picker>
					<div v-else-if="$check_field('get','release_time')">{{form['release_time']}}</div>
							</el-form-item>
			</el-col>
								<el-col v-if="user_group === '管理员' || $check_field('get','content_description') || $check_field('add','content_description') || $check_field('set','content_description')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="内容描述" prop="content_description">
								<el-input type="textarea" id="content_description" v-model="form['content_description']" placeholder="请输入内容描述"
						v-if="user_group === '管理员' || (form['message_board_id'] && $check_field('set','content_description')) || (!form['message_board_id'] && $check_field('add','content_description'))" :disabled="disabledObj['content_description_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','content_description')">{{form['content_description']}}</div>
							</el-form-item>
			</el-col>
	...
	export default {
		...
		data() {
			return {
				...
				url_upload: "~/api/message_board/upload?",
		...
		methods: {
          ...
          this.form["release_time"] = null;
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/message_board/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/message_board/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/message_board/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/message_board/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/message_board/view','get');
