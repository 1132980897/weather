        ...
        <div class="row">
          <div class="col-12">
            ...
            <div class="overflow-auto" v-else>
             ...
             <!-- 推荐气象数据模块(开始) -->
    <div class="warp" v-if="$check_action('/meteorological_data/list', 'get')">
      <!-- 容器 -->
      <div class="container">
        <div class="row">
          <div class="col-12">
            ...
            <div class="diy_recommend">
              ...
              <div class="more_box">
                <router-link to="/meteorological_data/list" class="more">
                  <span>More</span>
                </router-link>
              </div>
            </div>
            <!-- 气象数据推荐列表组件 -->
            <list_meteorological_data v-if="show_list_meteorological_data" :list="list_meteorological_data" />
            <div class="overflow-auto" v-else>
              <table
                id="list_diy"
                role="table"
                aria-busy="false"
                :aria-colcount="fields_meteorological_data.length"
                class="table b-table table-striped table-hover"
              >
                <thead>
                  <tr>
                    <th v-for="(o,i) in fields_meteorological_data" :key="i">
                      {{ o.label }}
                    </th>
                  </tr>
                </thead>
                <tbody>
                  <tr
                    v-for="(o, i) in list_table_meteorological_data"
                    :key="i"
                    @click="to_details('meteorological_data', o, 'meteorological_data_id')"
                  >
                    <td v-for="(oj,n) in fields_meteorological_data">
                      <img
                        v-if="oj.type && oj.type == '图片'"
                        :src="$fullUrl(o[oj.key])"
                        alt=""
...
<script>
	import mixin from "@/mixins/page.js";
				import list_meteorological_data from "@/components/diy/list_meteorological_data.vue";
	...
	export default {
        ...
        "list_type_commodity_type": [],
				...
				show_list_article: true,
							show_list_meteorological_data: true,
				list_meteorological_data: [],
				vm_arr_meteorological_data: [
					"meteorological_data_id",
							...
							"precipitation",
							"extreme_wind_speed",
							"a_direction_of_extreme_wind_speed",
							...
							"sunlight_hours",
							"minimum_air_pressure",
							"minimum_temperature",
							"maximum_air_pressure",
							"maximum_temperature",
							"maximum_wind_speed",
							"wind_direction_of_maximum_wind_speed",
							"minimum_relative_humidity",
						],
				fields_meteorological_data: [
										{
						key: "hohhot",
						label: "呼和浩特",
						type: "文本"
						...
						type: "文本"
													...
													{
						key: "extreme_wind_speed",
						label: "极大风速",
						type: "文本"
					},
													{
						key: "a_direction_of_extreme_wind_speed",
						label: "极大风速的风向",
						type: "文本"
					},
													{
						key: "average_air_pressure",
						label: "平均气压",
						type: "文本"
					},
													{
						...
						type: "文本"
													...
													{
						key: "maximum_wind_speed",
						label: "最大风速",
						type: "文本"
					},
													{
						key: "wind_direction_of_maximum_wind_speed",
						label: "最大风速的风向",
						type: "文本"
					},
													{
		...
		methods: {
			...
			get_meteorological_data() {
				let url = "~/api/meteorological_data/get_list?";
										this.$get(url, {
									"page": 1,
					"size": 8
				}, (json) => {
					if (json.result) {
						this.list_meteorological_data = json.result.list;
					}
				})
			},
		...
		computed: {
					    list_table_meteorological_data() {
		        var list = this.list_meteorological_data;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_meteorological_data.map((o) => {
		                // 第二个中括号是对象的属性
