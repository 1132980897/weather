<template>
	<div class="diy_details page_meteorological_data" id="meteorological_data_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_meteorological_data'>
							<div_meteorological_data :obj="obj"></div_meteorological_data>
...
<script>
	import div_meteorological_data from "@/components/diy/div_meteorological_data.vue";
	...
	export default {
		...
		components:{
			...
			bar_title,
			form_editor,
			div_meteorological_data
		},
		data(){
			return{
				url_get_obj: "~/api/meteorological_data/get_obj?",
				...
				form_comment: {
					...
					source_field: "meteorological_data_id",
					source_id: 0,
					reply_to_id: 0,
				},
				count_pages: 1, // 页面数
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/meteorological_data/set?meteorological_data_id=' + obj["meteorological_data_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "meteorological_data",
							source_field: "meteorological_data_id",
							source_id: this.obj.meteorological_data_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.meteorological_data_id;
				}
			},
			/**
			 * 分页器
